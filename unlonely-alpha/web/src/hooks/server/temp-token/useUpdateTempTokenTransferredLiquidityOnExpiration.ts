...
import { UpdateTempTokenTransferredLiquidityOnExpirationMutation, UpdateTempTokenTransferredLiquidityOnExpirationMutationVariables } from "../../../generated/graphql";
...
const MUTATION = gql`
mutation UpdateTempTokenTransferredLiquidityOnExpiration($data: UpdateTempTokenTransferredLiquidityOnExpirationInput!) {
...
const useUpdateTempTokenTransferredLiquidityOnExpiration = ({ onError }: Props) => {
    const [loading, setLoading] = useState(false);
    const [mutate] = useAuthedMutation<UpdateTempTokenTransferredLiquidityOnExpirationMutation,
    ...
    const updateTempTokenTransferredLiquidityOnExpiration = useCallback(
      async (data) => {
        try {
          ...
          const res = mutationResult?.data?.updateTempTokenTransferredLiquidityOnExpiration;
          if (res) {
            console.log("updateTempTokenTransferredLiquidityOnExpiration success", res);
          } else {
            onError && onError();
          }
          setLoading(false);
