...
export const useGetHighestTotalSupply = (contract: ContractData) => {
  const publicClient = usePublicClient();
  const [highestTotalSupply, setHighestTotalSupply] = useState<bigint>(
    BigInt(0)
  );
...
export const useGetCreationBlockNumber = (contract: ContractData) => {
  const publicClient = usePublicClient();
  const [creationBlockNumber, setCreationBlockNumber] = useState<bigint>(
    BigInt(0)
  );
...
export const useGetHasHitTotalSupplyThreshold = (contract: ContractData) => {
  const publicClient = usePublicClient();
  const [hasHitTotalSupplyThreshold, setHasHitTotalSupplyThreshold] =
...
export const useMint = (
  ...
  const {
    ...
    {
      value: args.value,
      enabled: contract.address !== NULL_ADDRESS && contract.abi !== null,
    }
  );
...
export const useUpdateTotalSupplyThreshold = (
  ...
  const {
    ...
    createCallbackHandler(
      "useTempTokenV1 updateTotalSupplyThreshold",
      callbacks
    ),
    {
      enabled: contract.address !== NULL_ADDRESS && contract.abi !== null,
...
export const useSendRemainingFundsToWinnerAfterTokenExpiration = (
  ...
  const {
    ...
    createCallbackHandler(
      "useTempTokenV1 sendRemainingFundsToWinnerAfterTokenExpiration",
      callbacks
    ),
    {
      enabled: contract.address !== NULL_ADDRESS && contract.abi !== null,
