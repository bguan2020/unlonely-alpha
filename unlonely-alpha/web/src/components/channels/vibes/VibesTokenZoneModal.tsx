...
const VibesTokenZoneModal = ({
  ...
  const _handleClose = () => {
    if (channelVibesTokenPriceRange.length === 0)
      setSliderValue([formattedCurrentPrice, formattedCurrentPrice]);
    if (channelVibesTokenPriceRange.length === 2)
      setSliderValue(channelVibesTokenPriceRange);
    handleClose();
  };
        ...
        {!loading ? (
          <Flex direction="column" gap="10px">
            <Button
              ...
              isDisabled={
                ...
                (Number(sliderValue[0]) ===
                  Number(channelVibesTokenPriceRange[0]) &&
                  Number(sliderValue[1]) ===
                    Number(channelVibesTokenPriceRange[1]))
              }
              onClick={() => submit()}
            >
              {Number(sliderValue[0]) > Number(sliderValue[1])
            ...
            <Button
              ...
              onClick={() => submit(true)}
              isDisabled={channelVibesTokenPriceRange.length === 0}
            >
              clear zones
            </Button>
          </Flex>
