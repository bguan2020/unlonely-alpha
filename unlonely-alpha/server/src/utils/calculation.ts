...
export const calculateEthFromVibesAmount = (
  buyOrSell: VibesTransactionType,
  vibesAmount: bigint,
  ...
  if (buyOrSell === VibesTransactionType.BUY) {
    ...
    const calculatedBurnProceeds = burnProceeds(
      vibesAmount,
      totalVibesSupplyBeforeSell
    );
    protocolFee =
      (calculatedBurnProceeds * protocolFeePercentage) / BIG_INT_ETHER;
    streamerFee =
      (calculatedBurnProceeds * streamerFeePercentage) / BIG_INT_ETHER;
...
export const mintCost = (amount: bigint, totalSupplyBeforeBuy: bigint) =>
  sumOfPriceToNTokens(totalSupplyBeforeBuy + amount) -
  sumOfPriceToNTokens(totalSupplyBeforeBuy);

export const burnProceeds = (amount: bigint, totalSupplyBeforeSell: bigint) =>
  sumOfPriceToNTokens(totalSupplyBeforeSell) -
  sumOfPriceToNTokens(totalSupplyBeforeSell - amount);

const sumOfPriceToNTokens = (n: bigint) =>
