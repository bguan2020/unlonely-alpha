...
const flattenedSoliditySourceCodeString = `// Sources flattened with hardhat v2.18.2 https://hardhat.org
...
export const verifyTempTokenV1OnBase = async (tempTokenContractAddress: `0x${string}`, encodedConstructorArguments: string): Promise<{
    status: string,
    message: string,
    result: string
}> => {
    ...
    console.log(tempTokenContractAddress, encodedConstructorArguments)
    ...
    return await axios.post(url, {
        ...
        headers: {
            "Authorization": "Bearer YI2EP6PXGKQ614AJABB3W5FRG8TSG539E9"
          }
    }).then(async (res) => {
        ...
        if (status === "1") {
            ...
            return await checkVerificationStatusOnBase(result)
        } else {
            return { status: "error", message: "An error occurred while verifying the contract on BaseScan", result: "" }
        }
    }).catch((err) => {
        console.error("verifyTempTokenV1OnBase", err)
        return { status: "error", message: "An error occurred while verifying the contract on BaseScan", result: "" }
    })
}
...
export const checkVerificationStatusOnBase = async (globalUniqueIdentifierForVerification: string) => {
    ...
    try {
        ...
        console.error(err);
        return { status: "error", message: "An error occurred while checking the verification status on BaseScan", result: "" };
    }
}